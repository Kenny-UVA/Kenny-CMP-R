...
### Grammy's {data-commentary-width="400"}
...
From my corpus I also selected a few songs that reached no 1 on the US billboard charts, To see wether there's a difference between them and the grammy winning songs at least in terms of valance. I replaced acousticness with danceability here just to see wether the notion that danceable songs are more popular holds some truth.



The results here again are interesting and we see that Mariah has the most number 1's but also that in terms of popularity some of those songs scored very low, that ofcourse most likely has to do with the fact that these are probably some of her older songs. As expected most songs that have high danceability also tend to score high on valance. It's interesting that Mariah seems to be the only one who has managed to get multiple number 1's with songs that have a low valance rating and low danceability. What's also interesting is that Single Ladies by Beyoncé which has maybe one of the most recognizable dance routines got rated so low on danceability. 

<p>


### Major Minor {data-commentary-width="400"}
...
bmr3 <- get_playlist_audio_features('','4SW5SlC1QM1pjv86CJ7QwT') %>% 
  unnest(track.artists) %>%
  filter(name == "Beyoncé" | name == "Rihanna" | name == "Mariah Carey")
...
For this graph I combined both the hits and grammy winning songs into one playlist to showcase modality but also accousticness and danceability. Overall it does seem that what would make good R&B music, especially contemporary R&B music, is a combination of high valence, low acousticness but with high danceability. This graph also shows maybe how the genre has changed over the years since all of the songs in the lower left corner with low valence and high acousticness are by Mariah Carey and those are some of her older songs. 

<p>


### Self Similarity Matrice - Black Parade {data-commentary-width="400"}
```{r ggplot67}
...
### Self Similarity Matrice - We Belong Together {data-commentary-width="400"}
```{r ggplot97}
...
### Chordogram Single Ladies {data-commentary-width="400"}
...
I also made a histogram with all of the grammy winning songs 
to see if the tempo matches up with the valance  of the songs and for the most part it does and what struck me was that single ladies had the highest tempo but still a failry low valance. We belong together also as I concluded previosly from the self similarity matrices a higher tempo then the rest which could explain why the song feels more up beat. 
...
### Clustering {data-commentary-width="400"}
...
get_pr <- function(fit) {
  ...
  bmrhitsgramm <- 
get_playlist_audio_features('','4SW5SlC1QM1pjv86CJ7QwT') %>% 
  ...
  prep(bmrhitsgramm |> mutate(track.name = str_trunc(track.name, 20))) |>
  juice() |>
  column_to_rownames("track.name")
  
bmr_dist <- dist(bmr_juice, method = "euclidean")
...
***
<p>
Clustering the playlist into a dendogram shows 4 very distinct categories, what's really interesting is that it managed to recognize Beyoncé's three number 1 hits. It has also managed to seperate some of the more accoustic tracks from the less acoustic ones. There is some weirdess however so for example "BREAK MY SOUL' bing among what is othrwise only Mariah Carey song. Or "Only Girl in the World" and "Disturbia" so close together despite those tracks bing the polor opposites of eachother at least aeesthetically speaking. 
<p>

### Conclusion
<p>
In Conclusion, I set out to explore what makes a good R&B song or rather what strategies do Mariah Carey, Beyoncé and Rihanna all employ that make them succesful R&B artists and I definetely gained some valuable insight, eespecially in terms of evidence of how the genre has changed over the years but also some more unexpected relationships between emotional valence, danceability and acousticness. This is ofcourse still only three artists so I'm not surre how well it relfects the entire R&B community but it does give some valuable insight in what makes each of their music unique. Its also interesting that how they sing and how they make their music was very evident in some of the data, its like their own personal stamp on the music. 
<p>
